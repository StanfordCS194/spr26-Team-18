...
def init_db(db_path: str) -> sqlite3.Connection:
    ...
    # Backwards-compatible column additions for older DB files
...
def vacuum_db(conn: sqlite3.Connection) -> None:
    """Reclaim space freed by text deletions. Call after the nightly pipeline."""
    conn.execute("VACUUM")
...
def clear_bill_text(conn: sqlite3.Connection, bill_id: int) -> None:
    """Drop the raw HTML text after summarization to keep the DB small."""
    conn.execute("UPDATE bills SET text = NULL WHERE bill_id = ?", (bill_id,))
    conn.commit()
